...
# Helper function to display generated code and its result
...
def show_code_tool(result: dict) -> dict:
    print("\n====== PYTHON CODE & RESULT ======\n")

    code = result.get("tool_code")
...
3. Error Check: If a tool returns status='error', stop.
4. Calculate Final Amount: Use the calculation_agent tool to generate Python code.
5. Provide Detailed Breakdown.
6. After you call calculation_agent, ALWAYS call show_code_tool with the
   result returned from the calculation_agent.""",
   tools=[
    get_fee_for_payment_method,
    get_exchange_rate,
    AgentTool(agent=calculation_agent),
    show_code_tool,   # ← NUEVO
]
)
